...
class FastDeconv(conv._ConvNd):
    ...
    def forward(self, x):
        ...
        if self.training and (not frozen):
            ...
            if self.groups==1:
                #Cov = X.t() @ X / X.shape[0] + self.eps * torch.eye(X.shape[1], dtype=X.dtype, device=X.device)
                Id=torch.eye(X.shape[1], dtype=X.dtype, device=X.device).cuda()
                Cov = torch.addmm(self.eps, Id, 1. / X.shape[0], X.t(), X).cuda()
                deconv = isqrt_newton_schulz_autograd(Cov, self.n_iter).cuda()
            else:
